...
el.innerHTML = `<div class="empty">${escapeHtml(data?.message || '沒有可用資料')}</div>`;
return;
}

el.innerHTML = data.windows
.map((item) => {
...
renderSection(codexLimitsEl, codex);
renderSection(claudeLimitsEl, claude);

const checkedText = formatClock(codex?.checkedAt || codex?.updatedAt || claude?.checkedAt);
...
`Claude 來源：${claude?.sourceType || 'unknown'}`,
`Claude 路徑：${claude?.sourcePath || '--'}`,
`Claude 狀態：${claude?.ok ? (claude?.stale ? '資料偏舊，建議手動更新檔案' : '資料新鮮') : (claude?.message || '尚未設定')}`
].join('\n');
